...
We can also add a new vector containing the symbols and those symbols :
```cpp
// File : reservedEntries.hpp
...
The entry `entrypoint` is defined as a keyword, it should be a type `TokenType::KEYWORD` or `4`, why it is not ? Let's find out <br>
